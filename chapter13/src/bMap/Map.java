 ...
 */
//Map接口
//Map与Collection并列存在 用于保存具有映射关系的数据 即k-v键值对
//之前的HashSet其实也是存的k-v 只不过value是一个固定的值 而在这里可以自定义
//key 和 value可以是任何类型的数据 会被封装到HahMap$Node对象中(熟悉吗)
//    key就是那个对象
    //与HashSet一样添加与取出顺序不一致 但每次取出都一致
    //线程不安全
//基本可以认为和HashSet一样 因为HashSet调的也是HashMap
//Map中的key不能重复(重复时会替换(覆盖)value) value可以重复
...
//但其实k-v为了方便遍历 会创建entrySet（EntrySet类型 集合的名字叫entrySet）集合 这个集合定义的类型是Map.Entry（Entry是Map里的一个接口）
//但实际上传入的还是HashMap$Node类型
...
//而且因为用Set类型接收 所以可以用构造器  进而方便了遍历
      //无关放一下先final class EntrySet extends AbstractSet<Map.Entry<K,V>>在HashMap里  ? ?
//弹幕说的
//一般老师会把基本信息(Node)做成 电子名片(Entry)的“形式”(实际还是Node)。然后把电子名片(Entry)通过entrySet()方法，记录在学生教育系统(EntrySet)中，
//为什么要这么做？？1.因为学生教育系统(Set接口类型)有遍历电子名片的功能(迭代器)。2.电子名片(Entry”类型”)有getKey()和getValue()功能就可以方便查询学生的学号(k)和姓名(v)了。
class Test2{
    public Test2() {
    }
    ...
    public static void main(String[] args) {
        ...
        for (Object o :set) {
            Map.Entry entry=(Map.Entry) o;//拿出来用Entry向下转型  用不了Node是因为Node是默认修饰符 （导包也算不同包） 讲道理
            //Node里也有getkey 和getvalue但是Set<Map.Entry<K, V>> entrySet();Entry也没有修饰符
            //我也不知道为什么
...
//与HashTable类似 可以用于.properties文件中（长作为配置文件） 加载外部数据到Properties类对象并进行读取修改
