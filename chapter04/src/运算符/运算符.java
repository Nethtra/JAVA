...
public class 运算符 {
    public static void main(String[] args) {
        ...
        //即上面说的赋值是赋值给中间变量  **最终使用的，并不是变量本身，而是被赋了值的临时变量temp.**
//看这段代码
        int i = 0;
        for (int j = 0; j < 5; j++) {//
            i = i++;
            //后自增  首先i的值被赋给临时变量temp   然后i自增    最后使用temp的值赋给i  所以结果还是0，五个0
            //             temp=i                i=i+1            i=temp
...
class LogicOperator {
    public static void main(String[] args) {
        ...
        short z = 46;
        if ((z++ == 46) && (y = true)) z++;//48
        if ((x = false) || (++z == 49)) z++;//50           x=false是false的
...
class BitOperator {
    public static void main(String[] args) {
        ...
        //<<算数左移   符号可能变   但注意范围  比如Int类型  左移超过32位 实际会取模即 如果左移33位就33%32=1 位移一位
