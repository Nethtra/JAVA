...
//数值默认是int     变long需要加l/L
//浮点默认是double  变float需要加f/F
//浮点型可能丢失精度的是尾数位
class num {
    public static void main(String[] args) {
...
//char类型 存储时存储数值 输出时默认输出unicode码对应的字符 本质是整数 可以进行运算
//可以存汉字
class ch {
    public static void main(String[] args) {
        char c1 = 'a';
...
//细节  多种类型混合**运算时**系统会先将所有类型转成(右边)最大的再计算   例1
//       1赋值时大的赋给小的会报错  反之自动转换
//       2(byte short)和char间不会自动转换 如上顺序所示 分了两条 就是这么规定的 例3
//       3byte short char 三者可以参与计算 参与时 一定会先升为int  例4
//       4boolean不参与自动转换
class AutoConvert {
    public static void main(String[] args) {
        int a = 'c';
...
//基本类型转字符串 直接拼接+“”就行
        ...
        System.out.println("=============");
//字符串转基本数据类型（首先确保能转换）   通过基本类型包装类调用parsexx方法 然后用对应类型接收
