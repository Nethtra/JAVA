package chapter08;
//encapsulation
//将抽象出的属性和方法封装在一起 数据被保护在内部 程序的其他部分只有通过授权的操作方法才能堆数据进行操作
//作用
//隐藏实现细节
//可以对数据进行验证
...
//通过一道题来理解封装的作用
//题干 创建程序定义两个类 Account和AccountTest Account类要求具有属性姓名（长度2-6）余额（>=20）
//密码（>=六位）,如果不满足给出提示信息并赋默认值
//在AccountTest中测试
class Account{
    //要满足数据安全性 所以封装
    //private化使只能通过我们自己的set方法来赋值
    private String name;
    ...
    //不提供无参构造器要是有有参构造器，创建类的时候就必须要初始化值 ，有时候不想初始化
    //有时候只是想实例化然后调用方法，并不想直接传参

    public Account(String name, double balance, String psw) {
        //this.name = name;
        //this.balance = balance;
        //this.psw = psw;   如果这样的话初始化调用构造器时还是可以绕过我们下面设的条件
        //就是你在其他类里new了对象后再 account.name=dingzhen是不可以的（因为private）
