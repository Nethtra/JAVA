...
//Enumeration
//当一个类的对象已经确定为有限的几个值而且需要只读不能修改时 则该类可以设计成枚举类 比如season季节类
//枚举是一种特殊类 一组常量的集合
//实现方式
//1自定义枚举
//属性、构造器私有化，去掉set方法，属性声明为private final
//在枚举类的内部创建枚举类的对象用 public static final修饰 对象名要大写
//这里联想一下单例设计模式
//2使用enum关键字
//用enum替代class，使用enum关键字后默认会继承一个叫Enum的类
//继承该类后，枚举成员都默认被public static final 修饰
//枚举对象（成员）（常量）写在最前
//属性、构造器私有化 删set方法
...
//Enum中的一些方法
//toString() 返回枚举常量名 可以在enum重写 如果不重写就和name一个效果
//name() 返回枚举常量名
